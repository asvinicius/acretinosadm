  ...
  <div class="row">
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Gerenciar tags</h1>
		<div class="btn-toolbar mb-2 mb-md-0">
			<div class="btn-group mr-2">
				<a class="btn btn-sm btn-outline-primary" href="<?= base_url('novatag'); ?>">
	  ...
	  <?php if($tags){ ?>
			<table class="table table-sm table-hover">
				...
				<tbody>
					<?php foreach($tags as $tag){ ?>
						<tr>
							<td><?php echo $tag->tagname ?></th>
							<td><?php echo $tag->tagslug ?></td>
							<td>
								<?php 
									if($tag->tagstatus == 1){
										echo "Ativa";
							...
							<td>
								<!--<a href="<?= base_url('#'.$category->categoryid); ?>" 
									title="<?php 
								...
								<?php 
									if($tag->tagstatus == 1){?>
										<a href="<?= base_url('tags/mudarstatus/'.$tag->tagid); ?>" title="Desativar"><span data-feather="toggle-right"></span></a>
									<?php } else { ?>
										<a href="<?= base_url('tags/mudarstatus/'.$tag->tagid); ?>" title="Ativar"><span data-feather="toggle-left" stroke="orange"></span></a>
									<?php } ?>
								<a href="<?= base_url('editatag/prepare/'.$tag->tagid); ?>" title="Editar"><span data-feather="edit-2"></span></a>
								<a href="<?= base_url('tags/delete/'.$tag->tagid); ?>" title="Apagar" onclick="return confirm('Tem certeza que deseja fazer isto?\nRemover uma tag pode causar problemas na filtragem das notícias.');"><span data-feather="trash-2" stroke="red"></span></a>
							</td>
						</tr>
					<?php } ?>
				</tbody>
		...
		<?php } else {
			echo "<h4>Nenhuma tag registrada.</h4>";
			} ?>
    </main>
  </div>
</div>
