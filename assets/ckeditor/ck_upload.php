...
if(isset($_FILES['upload']) && strlen($_FILES['upload']['name']) > 1) { 
    ...
    if(in_array($type, $imgset['type'])){ 
        ...
        if(isset($width) && isset($height)) { 
            if($width > $imgset['maxwidth'] || $height > $imgset['maxheight']){ 
                $re .= '\\n Width x Height = '. $width .' x '. $height .' \\n O dimensionamento máximo deve ser: '. $imgset['maxwidth']. ' x '. $imgset['maxheight']; 
            } 
 
            if($width < $imgset['minwidth'] || $height < $imgset['minheight']){ 
                $re .= '\\n Width x Height = '. $width .' x '. $height .'\\n O dimensionamento mínimo deve ser: '. $imgset['minwidth']. ' x '. $imgset['minheight']; 
            } 
 
            if($_FILES['upload']['size'] > $imgset['maxsize']*1000){ 
                $re .= '\\n O tamanho máximo do arquivo de ser: '. $imgset['maxsize']. ' KB.'; 
            } 
        } 
    }else{ 
        $re .= 'O arquivo: '. $_FILES['upload']['name']. ' tem uma extensão não permitida!\\nUtilize arquivos nas extensões jpg, jpeg ou png'; 
    } 
    ...
    if($re == ''){ 
        if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) { 
            ...
            $url = 'http://'.$_SERVER["REMOTE_ADDR"].'/sindicatoadm/assets/ckeditor/'. $upload_dir . $f_name; 
            $msg = F_NAME .'.'. $type .' foi enviada com sucesso: \\n- Tamanho: '. number_format($_FILES['upload']['size']/1024, 2, '.', '') .' KB'; 
